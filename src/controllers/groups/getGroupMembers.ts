...
export default async function getGroupMembersController(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "GET") {
        res.status(405).end()
        return
    }
    ...
    try {
        ...
        res.status(200).send({ data: leaves.map((leave) => leave.hash).reverse() })
    } catch (error) {
        res.status(500).end()
