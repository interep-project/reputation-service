import { getDefaultNetworkId, getDefaultNetworkName } from "./getDefaultNetwork"
...
describe("getDefaultNetworkId", () => {
    ...
    it("should return local hardhat network id for development", () => {
        jest.spyOn(process.env, "NODE_ENV", "get").mockReturnValueOnce("development")

        expect(getDefaultNetworkId()).toBe(31337)
    })

    it("should return ropsten chain id for production by default", () => {
        jest.spyOn(process.env, "NODE_ENV", "get").mockReturnValueOnce("production")
