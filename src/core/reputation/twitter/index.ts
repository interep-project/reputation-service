...
export const getTwitterUserReputation = async ({
    ...
    if (botometerData.display_scores?.universal?.overall) {
        twitterParameters.botometerOverallScore = botometerData.display_scores.universal.overall
    }

    twitterAccount.botometer = botometerData
    twitterAccount.basicReputation = calculateReputation(Platform.TWITTER, twitterParameters)
...
export const checkTwitterReputationByUsername = async (
    ...
    // Check if account is in database already
    const twitterAccount: ITwitterAccountDocument | null = await TwitterAccount.findByTwitterUsername(username)
