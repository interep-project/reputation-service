...
export async function getInterRepGroupsContractInstance(): Promise<InterRepGroups> {
    ...
    if (!address) {
        throw new Error("Address not provided for instantiating InterRepGroups contract")
    }

    return ((await ethers.getContractAt("InterRepGroups", address)) as unknown) as InterRepGroups
}
