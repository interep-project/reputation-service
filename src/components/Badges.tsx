...
export default function Badges({ tokens, mintFunction, burnFunction, unlinkFunction }: Properties): JSX.Element | null {
                    ...
                    <ListItem key={token.decimalId} className={classes.item}>
                        ...
                        <ListItemSecondaryAction>
                            {token.status === TokenStatus.NOT_MINTED ? (
                                <Button onClick={() => mintFunction(token._id)} size="small" color="primary">
                                    Mint
                                </Button>
                            ) : token.status === TokenStatus.MINTED ? (
                                <Button
                                    onClick={() => burnFunction(token.web2Provider, token.decimalId)}
                                    size="small"
                                    color="primary"
                                >
                                    Burn
                                ...
                                token.status === TokenStatus.BURNED && (
                                    <Button onClick={() => unlinkFunction(token)} size="small" color="primary">
                                        Unlink
                                    </Button>
                                )
                            )}
